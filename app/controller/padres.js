  ...
  Padre
      ...
      .exec(function (err, todosPadres) {
        if (err) {
          ...
          console.log(`JSON con padres que hecho pagos desde el banco:${req.banco}`);
          res.status(200).json(todosPadres);
        }
      });
  // }
...
exports.findOne = function (req, res) {
  ...
  Padre.findOne({'_id': padId}).populate('pagos')
      .then(padre => {
        if (padre == null) {
...
function nuevoPad(req) {
  ...
  if (!Array.isArray(newData.pagos)){ newData.pagos = [];}
  if (!Array.isArray(newData.hijos)){ newData.hijos = [];}
  return newData;
}
